...
function App() {
  ...
  const [files, setFiles] = useState(mapArr(initFiles))  // 代表所有的文件信息
  ...
  const saveData = (id, newTitle, isNew) => {
    ...
    if (isNew) {
      // 执行创建
      writeFile(path.join(savedPath, `${newTitle}.md`), files[id].body).then(() => {
        setFiles({ ...files, [id]: newFile })
      })
    } else {
      // 执行更新
      renameFile(path.join(savedPath, `${files[id].title}.md`), path.join(savedPath, `${newTitle}.md`)).then(() => {
        setFiles({ ...files, [id]: newFile })
      })
    }
